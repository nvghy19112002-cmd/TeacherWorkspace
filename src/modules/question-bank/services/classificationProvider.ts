...
async function callGemini(model: string, key: string, input: string, signal: AbortSignal): Promise<ClassificationResult> {
  ...
  const text = redact(await response.text(), key);
  const envelope = envelopeSchema.parse(JSON.parse(text) as unknown);
  const candidate = envelope.candidates?.[0];
  if (!candidate || candidate.finishReason !== 'STOP') throw new Error('AI chưa hoàn tất phân loại.');
  const payload = candidate.content?.parts.filter((part) => !part.thought).map((part) => part.text ?? '').join('') ?? '';
...
function promptInput(snapshot: BankSnapshot, question: Question): string {
  const nodes = snapshot.curriculumNodes.filter((node) => !node.archived).map(({ id, parentId, kind, code, name, description }) => ({ id, parentId, kind, code, name, description }));
  const outcomes = snapshot.learningOutcomes.filter((outcome) => outcome.active).map(({ id, nodeId, code, content, source }) => ({ id, nodeId, code, content, source }));
  return JSON.stringify({
    curriculum: { nodes, learningOutcomes: outcomes },
    question: { rawSource: question.rawSource, type: question.questionType, answer: question.answer, solution: question.solution },
    levelRules: {
...
export async function classifyQuestionOnline(snapshot: BankSnapshot, question: Question, signal: AbortSignal): Promise<{ result: ClassificationResult; model: string; keyId: string }> {
  ...
  for (let index = 0; index < keys.length; index += 1) {
    ...
    try {
      ...
      await saveKeyPool((current) => ({
        ...current,
        keys: current.keys.map((item) => item.id === meta.id ? { ...item, status: 'quota', lastCheckedAt: new Date().toISOString() } : item),
      }));
    }
  }
